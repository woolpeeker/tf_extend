...
def bboxes_nms(scores, bboxes, nms_threshold=0.5, keep_top_k=200, score_thres=float('-inf'), scope=None):
    ...
    with tf.name_scope(scope, 'bboxes_nms_single', [scores, bboxes]):
        ...
        idxes = tf.image.non_max_suppression(bboxes, scores,
                                             keep_top_k, nms_threshold, score_thres)
...
def bboxes_filter_center(labels, bboxes, margins=(0., 0., 0., 0.),
    ...
    with tf.name_scope(scope, 'bboxes_filter', [labels, bboxes]):
        ...
        mask = tf.logical_and(mask, tf.less(cy, 1. + margins[2]))
...
def bboxes_jaccard(bbox_ref, bboxes, name=None):
    ...
    with tf.name_scope(name, 'bboxes_jaccard'):
        ...
        def has_no_bboxes():
            size = tf.cast(tf.size(bbox_ref)/4,tf.int32)
            return tf.zeros(shape=size,dtype=tf.float32)
...
def bboxes_max_iou(bboxes_a,bboxes_b,scope='bboxes_anchors_max_iou'):
    """compute each bbox in bboxes_a's max iou with bboxes_b"""
    with tf.name_scope(scope):
        def get_cls(bbox):
            return tfe.bboxes_jaccard(bbox, bboxes_b)
        ...
        def has_no_bboxes():
            zero_cls = tf.zeros(tf.shape(bboxes_a)[0], tf.float32)
            return zero_cls,

        return tf.cond(pred=tf.shape(bboxes_b)[0] > 0,
                       true_fn=has_bboxes,
                       false_fn=has_no_bboxes)
