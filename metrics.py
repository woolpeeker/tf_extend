...
#===================general metrics====================================
def TP_FP_TN_FN(labels, predictions, scope='TP_FP_TN_FN'):
    with tf.name_scope(scope):
        depend = tf.assert_equal(tf.shape(labels), tf.shape(predictions))
        TP = tf.logical_and(labels, predictions)
        TP = tf.reduce_sum(tf.to_float(TP))
        FP = tf.logical_and(tf.logical_not(labels), predictions)
        FP = tf.reduce_sum(tf.to_float(FP))
        TN = tf.logical_and(tf.logical_not(labels), tf.logical_not(predictions))
        TN = tf.reduce_sum(tf.to_float(TN))
        FN = tf.logical_and(labels, tf.logical_not(predictions))
        FN = tf.reduce_sum(tf.to_float(FN))
        return {'TP':TP, 'FP':FP, 'TN':TN, 'FN':FN}

def accuracy(labels, predictions, scope='accuracy'):
    with tf.name_scope(scope):
        depend = tf.assert_equal(tf.shape(labels), tf.shape(predictions))
        with tf.control_dependencies([depend]):
            equals = tf.cast(tf.equal(labels, predictions), tf.float32)
            acc = tf.reduce_mean(equals)
        return acc

def precision(labels, predictions, scope='precision'):
    with tf.name_scope(scope):
        depend = tf.assert_equal(tf.shape(labels), tf.shape(predictions))
        TP = tf.logical_and(labels, predictions)
        TP = tf.reduce_sum(tf.to_float(TP))
...
def recall(labels, predictions, scope='recall'):
    with tf.name_scope(scope):
        depend = tf.assert_equal(tf.shape(labels), tf.shape(predictions))
        TP = tf.logical_and(labels, predictions)
        TP = tf.reduce_sum(tf.to_float(TP))
...
def bboxes_precision(gt_bboxes, pred_bboxes, thres, scope='bboxes_precision'):
    #support batch, [batch, num, 4]
    with tf.name_scope(scope):
        def fn(i, ta, gt_bboxes=gt_bboxes, pred_bboxes=pred_bboxes):
            gt_bboxes=gt_bboxes[i,...]
            pred_bboxes = pred_bboxes[i,...]
            gt_bboxes = tf.boolean_mask(gt_bboxes,
                                        tf.reduce_any(gt_bboxes > 1e-6, axis=-1, keepdims=False))
            pred_bboxes = tf.boolean_mask(pred_bboxes,
                                    tf.reduce_any(pred_bboxes > 1e-6, axis=-1, keepdims=False))
            cls, _ = tfe.encode_pred_bboxes(pred_bboxes, gt_bboxes)
            cls = tf.cast(cls > thres, tf.float32)
            ta = ta.write(i,cls)
            return i+1, ta

        # iter each batch
        i=0
        ta = tf.TensorArray(dtype=tf.float32, size=0, dynamic_size=True, clear_after_read=False, element_shape=[None],infer_shape=False)
        _, cls = tf.while_loop(cond=lambda i,_: i<tf.shape(gt_bboxes)[0],
                               body=fn,
                               loop_vars=[i, ta],
                               parallel_iterations=8,back_prop=False)
        cls = cls.concat()
        prec = tf.reduce_mean(cls)
        return prec

def bboxes_recall(gt_bboxes, pred_bboxes, thres, scope='bboxes_recall'):
    ...
    with tf.name_scope(scope):
        def fn(i, ta, gt_bboxes=gt_bboxes, pred_bboxes=pred_bboxes):
            ...
            pred_bboxes = tf.boolean_mask(pred_bboxes,
                                    tf.reduce_any(pred_bboxes > 1e-6, axis=-1, keepdims=False))
            cls, _ = tfe.encode_pred_bboxes(gt_bboxes, pred_bboxes)
            cls = tf.cast(cls > thres, tf.float32)
            ta = ta.write(i,cls)
            return i+1, ta
